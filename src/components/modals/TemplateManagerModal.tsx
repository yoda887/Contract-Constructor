...
import { X, Save, Plus, FileText, CheckCircle2, Trash2, FileUp } from 'lucide-react';
...
export const TemplateManagerModal: React.FC<TemplateManagerModalProps> = ({
  ...
  const handleCreateNewTemplate = (e: React.FormEvent) => {
    ...
    const roleA = newTemplate.partyARole || 'Сторона 1';
    const roleB = newTemplate.partyBRole || 'Сторона 2';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl border border-slate-200 space-y-5">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="font-black text-base text-slate-900">
              {mode === 'create' ? 'Создание нового шаблона' : 'Выбор шаблона договора'}
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        {/* EXISTING TEMPLATES (ONLY IN SELECT MODE) */}
        {mode === 'select' && (
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
              {onOpenDocxImport ? (
                <button
                  ...
                  className="bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold px-3 py-1.5 rounded-xl text-xs transition-colors flex items-center space-x-1.5 border border-blue-200 cursor-pointer"
                >
                  <FileUp className="w-4 h-4 text-blue-600" />
                  <span>Импорт из Word (.docx)</span>
                </button>
              ) : <span />}
        ...
        {/* FORM TO CREATE NEW TEMPLATE (ONLY IN CREATE MODE) */}
        {mode === 'create' && (
          <form onSubmit={handleCreateNewTemplate} className="space-y-3 text-xs">
            <div>
